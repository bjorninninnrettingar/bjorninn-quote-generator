...
async function buildOrderPdf(order, lines) {
  ...
  // ── Line items table ─────────────────────────────────────────────────────
  const cols = [
    { label: "Vara",       w: 0.52, align: "left",   clip: true  },
    { label: "Tegund",     w: 0.16, align: "left",   clip: true  },
    { label: "Magn",       w: 0.12, align: "center", clip: false },
    { label: "Áætl. kostn.", w: 0.20, align: "right",  clip: false },
  ];
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const item = lines[i];
    const name = item["Vörulisti txt"] || item["Vörunúmer #️⃣"] || "—";
    const type = item["Gerð"] || "";
    const qty  = item["Magn"] ?? "";

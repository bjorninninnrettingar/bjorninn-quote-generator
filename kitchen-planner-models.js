...
window.KPMODELS = {
  ...
  handles: {
    // Jey (grip): a full-width profile that REPLACES 27 mm of the front; the finger lip is at the BOTTOM of the profile
    // (model y = length, model z = height with the lip at the low end, model x = the ~17 mm it projects)
    jey:                { file:"models/handles/jey.glb",   kind:"jey", map:["-y", "-z", "x"], color:"#b8935a", stripMm:27 },
    ...
    // Vann: 100 mm flip on the top edge (first of the four lengths in the file: `take` = first 2 meshes)
    vann:               { file:"models/handles/vann.glb", kind:"topmount", map:["y", "x", "-z"], take:2, color:"#b87a55", lenMm:100 },
    ...
    crossing:           { file:"models/handles/crossing.glb", kind:"bar", map:["-z", "y", "x"], lenMm:328, fitMarginMm:100 },
    graf:               { file:"models/handles/graf-big.glb", kind:"bar", map:["y", "-x", "z"], posts:true, color:"#1e1d1d", lenMm:320, fitMarginMm:100 },
    sense:              { file:"models/handles/sense-big.glb", kind:"bar", map:["-z", "y", "x"], dropFlat:true, color:"#1a1a1c", lenMm:263, fitMarginMm:100 },
